...
unsigned short getBlockIdFromName(const std::string &name) {
   assertDebug(blockIds.map.count(name), "DEBUG: Block with the name '{}' does not exist!", name);
...
void Map::render(const std::vector<DroppedItem> &droppedItems, const Player &player, float accumulator, const Rectangle &cameraBounds, const Camera2D &camera, const Inventory &inventory) {
   ...
   size_t maximum = std::max(sandCount, std::max(forestCount, std::max(jungleCount, std::max(mountainCount, tundraCount))));
